...
-- Check if a player is in danger based on its health and energy level.
function isInDanger(player)
    return player.health < 20 and player.energy > 10
end
...
function decideNextAction()
  ...
  for target, state in pairs(LatestGameState.Players) do
    if target ~= ao.id and inRange(player.x, player.y, state.x, state.y, 1) then
      targetInRange = true
      -- Prioritize targets based on health, distance, and energy
  ...
  if isInDanger(player) then
    print(colors.blue .. "Low health! Attempting to flee." .. colors.reset)
    -- Flee by moving to a random direction
  ...
  elseif player.energy > 5 and targetInRange then
    ...
    -- Attack the closest player with all your energy.
    ao.send({
      Target = Game,
      Action = "PlayerAttack",
      Player = ao.id,
  ...
  elseif player.energy < 3 then
    print(colors.green .. "Low energy! Waiting for energy to regenerate." .. colors.reset)
    -- Wait for energy to regenerate
  else
    print(colors.red .. "No player in range or no immediate danger. Moving randomly." .. colors.reset)
    -- Move randomly if no target in range
    local directionRandom = {"Up", "Down", "Left", "Right"}
    local randomIndex = math.random(#directionRandom)
    ao.send({Target = Game, Action = "PlayerMove", Player = ao.id, Direction = directionRandom[randomIndex]})
  end
